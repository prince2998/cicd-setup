...
k8s-node2 ansible_host=  ansible_user=ubuntu ansible_connection=ssh
EOF
...
sed -i "s/^k8s-node2.*ansible_host=.*/k8s-node2 ansible_host=${node1_public_ip} ansible_user=ubuntu ansible_connection=ssh/" ../ansible_config/inventory
